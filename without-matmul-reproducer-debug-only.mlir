+ iree-compile /home/rdhar/export_mlir/iree/with-matmul-reproducer.mlir --iree-hal-target-device=hip --iree-hip-target=gfx942 --iree-opt-level=O3 --iree-opt-const-eval=false --iree-hip-waves-per-eu=2 --iree-llvmgpu-enable-prefetch --iree-dispatch-creation-enable-fuse-horizontal-contractions=true --iree-execution-model=async-external '--iree-preprocessing-pass-pipeline=builtin.module(iree-preprocessing-transpose-convolution-pipeline,iree-preprocessing-pad-to-intrinsics{pad-target-type=conv})' --mlir-disable-threading --debug-only=iree-global-opt-propagate-linalg-transpose -o punet.vmfb
...
util.func public @two_matmuls_same_transpose_rhs(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view, %arg3: !hal.fence, %arg4: !hal.fence) -> (!hal.buffer_view, !hal.buffer_view) attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "async func @two_matmuls_same_transpose_rhs(%input0: tensor<64x768xf16>, %input1: tensor<64x768xf16>, %input2: tensor<768x768xf16>) -> (%output0: tensor<64x768xf32>, %output1: tensor<64x768xf32>)", iree.abi.model = "coarse-fences"}} {
  ...
  %5 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d2, d1)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%0, %transposed : tensor<64x768xf16>, tensor<768x768xf16>) outs(%4 : tensor<64x768xf32>) {
  ...
  %6 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d2, d1)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%1, %transposed : tensor<64x768xf16>, tensor<768x768xf16>) outs(%4 : tensor<64x768xf32>) {
  ^bb0(%in: f16, %in_0: f16, %out: f32):
    %10 = arith.extf %in : f16 to f32
    %11 = arith.extf %in_0 : f16 to f32
    %12 = arith.mulf %10, %11 : f32
...
warning: <unknown>:0:0: failed to meet occupancy target given by 'amdgpu-waves-per-eu' in 'two_matmuls_same_transpose_rhs_dispatch_0_matmul_like_64x768x768_f16xf16xf32': desired occupancy was 2, final occupancy is 1
